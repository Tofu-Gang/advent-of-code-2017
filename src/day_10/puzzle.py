...
def puzzle_01() -> None:
    ...
    :return: None; Answer should be 3770.
    """

    with open(INPUT_FILE_PATH, "r") as f:
        lengths = f.read().strip()
        marks = tuple(i for i in range(256))
        knot_hash = KnotHash(lengths, marks)
        print_puzzle_solution(knot_hash.multiplication)
...
def puzzle_02() -> None:
    ...
    31,73,47,23 (the ASCII codes from the input string combined with the
    ...
    (including a leading zero as necessary). So, if your first three numbers are
    64, 7, 255, they correspond to the hexadecimal numbers 40, 07, ff, and so
    the first six characters of the hash would be 4007ff. Because every Knot
    ...
    :return: None; Answer should be a9d0e68649d0174c8756a59ba21d4dc6.
    """

    with open(INPUT_FILE_PATH, "r") as f:
        knot_hash = KnotHash(f.read().strip(), tuple(i for i in range(256)))
        print_puzzle_solution(knot_hash.knot_hash)
