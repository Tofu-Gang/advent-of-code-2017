...
from src.day_15.judge import Judge
...
PAIRS_COUNT_1 = 40000000
PAIRS_COUNT_2 = 5000000
...
def puzzle_01() -> None:
    ...
    :return: None; Answer should be 600.
    """

    with open(INPUT_FILE_PATH, "r") as f:
        line_a, line_b = f.readlines()
        pattern = compile(r"\d+")
        value_a = int(pattern.findall(line_a)[0])
        value_b = int(pattern.findall(line_b)[0])

        gen_a = DuelingGenerator(value_a, FACTORS[GENERATOR_A])
        gen_b = DuelingGenerator(value_b, FACTORS[GENERATOR_B])
        judge = Judge(gen_a, gen_b, PAIRS_COUNT_1)
        judge.generate_pairs()
        print_puzzle_solution(judge.matches)
...
def puzzle_02() -> None:
    ...
    :return: None; Answer should be 313.
    """
    ...
    with open(INPUT_FILE_PATH, "r") as f:
        ...
        gen_a = DuelingGenerator(
            value_a, FACTORS[GENERATOR_A], MULTIPLES[GENERATOR_A])
        gen_b = DuelingGenerator(
            value_b, FACTORS[GENERATOR_B], MULTIPLES[GENERATOR_B])
        judge = Judge(gen_a, gen_b, PAIRS_COUNT_2)
        judge.generate_pairs()
        print_puzzle_solution(judge.matches)
